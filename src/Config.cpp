...
float OFFSET{1};
DHTSensor dhtSensor(DHT_DATA_PIN_CONFIG, DHT_TYPE_CONFIG);

//  HMD Sensor parameters
float HMD_THRESHOLD = 60;
const uint8_t HMD_DATA_PIN_CONFIG = 3;
const uint8_t HMD_POWER_PIN_CONFIG = 7;
HMDSensor hmdSensor(HMD_POWER_PIN_CONFIG, HMD_DATA_PIN_CONFIG);

//  Fan parameters
const uint8_t FAN_PIN_CONFIG{4};
Fan fanMotor(FAN_PIN_CONFIG, false);

// Pump parameters
const uint8_t PUMP_PIN_CONFIG{5};
Pump pumpMotor{PUMP_PIN_CONFIG, "Pump1"};
...
MyServo servoMotor(SERVO_PIN_CONFIG, HZ_CONFIG, MIN_PULSE_WIDTH_CONFIG, MAX_PULSE_WIDTH_CONFIG);